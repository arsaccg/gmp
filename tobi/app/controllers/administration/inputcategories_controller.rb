...
class Administration::InputcategoriesController < ApplicationController
	...
	def feo_of_work
		...
		@budget_sale = Budget.find(params[:budget_sale]) rescue Budget.where(project_id: project_id).last
		@budget_goal = Budget.find(params[:budget_goal]) rescue Budget.where(project_id: project_id).last
