...
DROP PROCEDURE IF EXISTS `SHOWME_MACRO_VALUES_BI`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `SHOWME_MACRO_VALUES_BI`(IN vi_cost_center_id INT, IN vi_start_date CHAR(10), IN vi_end_date CHAR(10))
BEGIN
  ...
  BLOCKVAL: BEGIN
    ...
    DECLARE valorizations CURSOR FOR
      ...
      AND v.valorization_date BETWEEN vi_start_date AND vi_end_date;
  ...
  BLOCK5dc: BEGIN
    ...
    read_loop5dc: LOOP
      ...
      IF done5dc THEN
        LEAVE read_loop5dc;
      END IF;
      SET @real_cost_hand_work = (SELECT SUM(ppd.total_hours) FROM part_people pp, part_person_details ppd, phases p WHERE ppd.part_person_id = pp.id AND ppd.worker_id = v_worker AND pp.blockweekly = 1 AND pp.cost_center_id = vi_cost_center_id AND ppd.phase_id = p.id AND pp.date_of_creation BETWEEN v_date_begin AND v_date_end AND p.code < '90__' GROUP BY ppd.worker_id);
    ...
    read_loop5dcpw: LOOP
      ...
      END IF;
      SET @real_cost_hand_work = (SELECT SUM(8.5) FROM part_workers pw, part_worker_details pwd, phases p WHERE pwd.part_worker_id = pp.id AND pwd.worker_id = v_worker AND pw.blockweekly = 1 AND pw.cost_center_id = vi_cost_center_id AND pwd.phase_id = p.id AND pw.date_of_creation BETWEEN v_date_begin AND v_date_end AND p.code < '90__' GROUP BY pwd.worker_id);
      IF @real_cost_hand_work != NULL THEN
        SET real_cost_hand_work = real_cost_hand_work + v_total_h/@real_cost_hand_work*v_neto;
      END IF;
  ...
  BLOCKPROG: BEGIN
    ...
    AND di.month BETWEEN vi_start_date AND vi_end_date
    AND d.cost_center_id = vi_cost_center_id
  ...
  BLOCK5: BEGIN
    ...
    read_loop5: LOOP
      ...
      IF done5 THEN
        LEAVE read_loop5;
      END IF;
      SET @r_hand_work = (SELECT SUM(ppd.total_hours) FROM part_people pp, part_person_details ppd, phases p WHERE ppd.part_person_id = pp.id AND ppd.worker_id = v_worker AND pp.blockweekly = 1 AND pp.cost_center_id = vi_cost_center_id AND ppd.phase_id = p.id AND pp.date_of_creation BETWEEN v_date_begin AND v_date_end AND p.code LIKE  '90__' GROUP BY ppd.worker_id);
    ...
    read_loop5pw: LOOP
      ...
      IF done5 THEN
        LEAVE read_loop5pw;
      END IF;
      SET @r_hand_work = (SELECT SUM(8.5) FROM part_workers pw, part_worker_details pwd, phases p WHERE pwd.part_worker_id = pp.id AND pwd.worker_id = v_worker AND pw.blockweekly = 1 AND pw.cost_center_id = vi_cost_center_id AND pwd.phase_id = p.id AND pw.date_of_creation BETWEEN v_date_begin AND v_date_end AND p.code LIKE '90__' GROUP BY pwd.worker_id);
      IF @r_hand_work != NULL THEN
        SET r_hand_work = r_hand_work + v_total_h/@r_hand_work*v_neto;
      END IF;
  ...
  BLOCKGE: BEGIN
    ...
    DECLARE meta CURSOR FOR 
      ...
      WHERE ge.cost_center_id = vi_cost_center_id
      AND ge.id = ged.general_expense_id
      AND ge.code_phase > 90
      AND DATE_FORMAT( ge.created_at,  '%Y-%m-%d' ) BETWEEN vi_start_date AND vi_end_date
      GROUP BY ged.type_article;
  ...
  BLOCKPAY: BEGIN
    ...
    read_loop11: LOOP
      ...
      IF donepay THEN
        LEAVE read_loop11;
      END IF;
      SET @real_hand_work = (SELECT SUM(ppd.total_hours) FROM part_people pp, part_person_details ppd, phases p WHERE ppd.part_person_id = pp.id AND ppd.worker_id = v_worker AND pp.blockweekly = 1 AND pp.cost_center_id = vi_cost_center_id AND ppd.phase_id = p.id AND pp.date_of_creation BETWEEN v_date_begin AND v_date_end AND p.code > '91__' GROUP BY ppd.worker_id);
    ...
    read_loop5pw: LOOP
      ...
      IF donepay THEN
        LEAVE read_loop5pw;
      END IF;
      SET @real_hand_work = (SELECT SUM(8.5) FROM part_workers pw, part_worker_details pwd, phases p WHERE pwd.part_worker_id = pp.id AND pwd.worker_id = v_worker AND pw.blockweekly = 1 AND pw.cost_center_id = vi_cost_center_id AND pwd.phase_id = p.id AND pw.date_of_creation BETWEEN v_date_begin AND v_date_end AND p.code > '91__' GROUP BY pwd.worker_id);
      IF @real_hand_work != NULL THEN
        SET real_hand_work = real_hand_work + v_total_h/@real_hand_work*v_neto;
      END IF;
    END LOOP read_loop5pw;
