...
@purchaseOrderDetails.each do |data|
  ...
  pad(1) {
    table([ ["#{index}", "#{data.delivery_order_detail.article.code}", "#{data.delivery_order_detail.article.name}", "#{data.delivery_order_detail.center_of_attention.abbreviation}", "#{data.delivery_order_detail.sector.code}", "#{data.delivery_order_detail.phase.code}", "#{data.delivery_order_detail.unit_of_measurement.symbol}", "#{sprintf("%.4f", data.amount.to_f)}", "#{number_to_currency(data.unit_price.to_f, unit: @purchaseOrder.money.symbol, precision: 4)}", "#{(data.discount_before.to_f/(data.amount.to_f*data.unit_price.to_f).to_f*100).round(1)}%", "#{number_to_currency(data.unit_price_before_igv.to_f, unit: @purchaseOrder.money.symbol, precision: 2)}"] ], :width => 770, :cell_style => {:border_color=> "ffffff"}, :column_widths => [35,60,275,35,40,40,30,60,90,50,55]) do
      columns(0).font_style = :bold
