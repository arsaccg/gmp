...
if @familiars == 0
  ...
  @worker_familiars.each do |data|
    table([ ["#{data.paternal_surname}", "#{data.maternal_surname}", "#{data.names}", "#{data.relationship}", "#{data.dayofbirth}", "#{data.dni}"] ], :width => 520, :cell_style => {:height => 18}, :column_widths => [100,100,120,70,70,60]) do
          style(columns(0..5), :align => :center)
            style(columns(0..5), :size => 8)
          end
  end
  (0..(5-("#{@worker_familiars.count}").to_i-1)).each do |something|
    table([ ["", "", "", "", "", ""] ], :width => 520, :cell_style => {:height => 18}, :column_widths => [100,100,120,70,70,60]) do
          style(columns(0..5), :align => :center)
          style(columns(0..5), :size => 8)
          columns(0..5).font_style = :bold
        end
...
if @otherstudies==0
  ...
  (0..(3-("#{@worker_otherstudies.count}").to_i-1)).each do |something|
    table([ ["", ""] ], :width => 520, :cell_style => {:height => 18}, :column_widths => [400,120]) do
          style(columns(0..1), :align => :center)
          style(columns(0..1), :size => 8)
        end
  end
...
bounding_box [bounds.left + 420, bounds.bottom + 788], :width  => bounds.width do
  table([ ["ARSAC-CMASS-F-063"],["Rev.: 01"],["Fecha: #{@date.strftime("%d/%m/%Y")}"],["Página: 2 de 2"] ], :width => 100, :cell_style => {:height => 16}, :column_widths => [100]) do
          style(columns(0), :align => :center)
...
if @workercontract.nil?
  ...
  table([ ["DISTRITO)", "PROVINCIA","DEPARTAMENTO"],["#{@worker.cost_center.district.to_s}", "#{@worker.cost_center.province.to_s}","#{@worker.cost_center.department.to_s}"] ], :width => 520, :cell_style => {:height => 16}, :column_widths => [170,170,180]) do
          style(columns(0..2), :align => :center)
