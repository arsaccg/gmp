...
bounding_box [bounds.left + 445, bounds.bottom + 683], :width  => bounds.width do
  table([ [""] ], :width => 75, :cell_style => {:height => 75}, :column_widths => [75])
end
bounding_box [bounds.left + 0, bounds.bottom + 683], :width  => bounds.width do
  table([ ["CÓDIGO", "APELLIDO PATERNO", "APELLIDO MATERNO","NOMBRES"] ], :width => 445, :cell_style => {:height => 21}, :column_widths => [60,100,100,185]) do
...
table([ ["#{@worker.id.to_s}", "#{@worker.entity.paternal_surname.to_s}", "#{@worker.entity.maternal_surname.to_s}","#{@worker.entity.name.to_s} #{@worker.entity.second_name.to_s}"] ], :width => 445, :cell_style => {:height => 18}, :column_widths => [60,100,100,185]) do
        style(columns(0..3), :align => :center)
        style(columns(0..3), :size => 8)
      end
table([ ["LUGAR DE NACIMIENTO(CIUDAD-PROVINCIA-DEPARTAMENTO)", "EDAD","FECH.NAC."] ], :width => 445, :cell_style => {:height => 18}, :column_widths => [285,80,80]) do
        style(columns(0..2), :align => :center)
        style(columns(0..2), :size => 8)
        columns(0..2).font_style = :bold
        style(row(0), :background_color => 'A0D8A0')
      end
table([ ["#{@worker.entity.city.to_s} #{@worker.entity.province.to_s} #{@worker.entity.department.to_s}", "#{@edad}","#{@worker.entity.date_of_birth.to_s}"] ], :width => 445, :cell_style => {:height => 18}, :column_widths => [285,80,80]) do
        style(columns(0..2), :align => :center)
        style(columns(0..2), :size => 8)
      end
...
if @worker.lastgrade.nil?
  table([ ["Situación Educativa", "#{@worker.levelofinstruction.to_s}"] ], :width => 160, :cell_style => {:height => 18}, :column_widths => [80,80]) do
  ...
  table([ ["Situación Educativa", "#{@worker.levelofinstruction.to_s}","","Último ciclo/año/grado cursado","#{@worker.lastgrade}"] ], :width => 520, :cell_style => {:height => 18}, :column_widths => [95,95,80,170,80]) do
          style(columns(0..4), :align => :center)
          style(columns(0..4), :size => 8)
          columns(0..4).font_style = :bold
...
if @experiencies == 0
  table([ ["NOMBRE DE LA EMPRESA","Cargo"],["",""] ], :width => 520, :cell_style => {:height => 18}, :column_widths => [260,260]) do
          style(columns(0..1), :align => :center)
          style(columns(0..1), :size => 8)
          columns(0..1).font_style = :bold
          style(row(0), :background_color => 'A0D8A0')
        end
  table([ ["SUELDO (S/.)", "JEFE INMEDIATO", "MOTIVO DE SALIDA", "DESDE", "HASTA"],["", "", "", "", ""] ], :width => 520, :cell_style => {:height => 18}, :column_widths => [100,140,120,80,80]) do
          style(columns(0..4), :align => :center)
          style(columns(0..4), :size => 8)
          columns(0..4).font_style = :bold
          style(row(0), :background_color => 'A0D8A0')
  ...
  @worker_experiences.each do |data|
    table([ ["NOMBRE DE LA EMPRESA","Cargo"],["#{data.businessname.to_s}","#{data.title.to_s}"] ], :width => 520, :cell_style => {:height => 18}, :column_widths => [260,260]) do
            style(columns(0..1), :align => :center)
            style(columns(0..1), :size => 8)
            columns(0..1).font_style = :bold
            style(row(0), :background_color => 'A0D8A0')
          end
    table([ ["SUELDO (S/.)", "JEFE INMEDIATO", "MOTIVO DE SALIDA", "DESDE", "HASTA"],["#{data.salary.to_s}", "#{data.bossincharge.to_s}", "#{data.exitreason.to_s}", "#{data.start_date.to_s}", "#{data.end_date.to_s}"] ], :width => 520, :cell_style => {:height => 18}, :column_widths => [100,140,120,80,80]) do
            style(columns(0..4), :align => :center)
            style(columns(0..4), :size => 8)
            columns(0..4).font_style = :bold
          style(row(0), :background_color => 'A0D8A0')
...
table([ ["CERTIFICADO DE CONVIVENCIA DE MATRIMONIO", if "#{@worker.marriage_certificate_file_size}"!="" then 'X' end, "DOCUMENTO NACIONAL DE IDENTIDAD", if "#{@worker.dni_file_size}"!="" then 'X' end] ], :width => 520, :cell_style => {:height => 18}, :column_widths => [240,20,240,20]) do
        style(columns(0..5), :align => :center)
        style(columns(0..5), :size => 8)
        columns(0..5).font_style = :bold
      end
...
if @workercontract.nil?
  ...
  table([ ["C.C","FECHA DE INGRESO", "CARGO", "CODIGO TOBI", "SUELDO (S/.)"],["", "", "", "", ""] ], :width => 520, :cell_style => {:height => 18}, :column_widths => [80,100,140,85,115]) do
          style(columns(0..4), :align => :center)
          style(columns(0..4), :size => 8)
          columns(0..4).font_style = :bold
          style(row(0), :background_color => 'A0D8A0')
  ...
  move_down 10
  table([ ["OBRA", "#{@worker.cost_center.cost_center_detail.name}"] ], :width => 520, :cell_style => {:height => 18}, :column_widths => [80,440]) do
          style(columns(0..1), :align => :center)
          style(columns(0..1), :size => 8)
          columns(0).font_style = :bold
          columns(1).font_style = :bold
  ...
  table([ ["DISTRITO)", "PROVINCIA","DEPARTAMENTO"],["#{@worker.cost_center.district.to_s}", "#{@worker.cost_center.province.to_s}","#{@worker.cost_center.department.to_s}"] ], :width => 520, :cell_style => {:height => 18}, :column_widths => [170,170,180]) do
          style(columns(0..2), :align => :center)
          style(columns(0..2), :size => 8)
          columns(0).font_style = :bold
          columns(1).font_style = :bold
  ...
  text "VI. DATOS CONTRACTUALES (LLENADO POR LA EMPRESA)", :size => 9
  table([ ["C.C.","FECHA DE INGRESO", "CARGO", "CODIGO TOBI", "SUELDO (S/.)"],["#{@worker.cost_center.code.to_s}","#{@workercontract.start_date.to_s}", "#{@workercontract.article.name.to_s}", "#{@workercontract.article.name.to_s}", "#{@workercontract.salary.to_s}"] ], :width => 520, :cell_style => {:height => 18}, :column_widths => [80,100,140,85,115]) do
          style(columns(0..4), :align => :center)
          style(columns(0..4), :size => 8)
          columns(0..4).font_style = :bold
          style(row(0), :background_color => 'A0D8A0')
  ...
  move_down 10
  table([ ["OBRA", "#{@worker.cost_center.cost_center_detail.name.to_s}"] ], :width => 520, :cell_style => {:height => 18}, :column_widths => [80,440]) do
          style(columns(0..1), :align => :center)
          style(columns(0..1), :size => 8)
          columns(0).font_style = :bold
          columns(1).font_style = :bold
