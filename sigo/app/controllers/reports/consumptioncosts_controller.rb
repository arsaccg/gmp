    ...
    if !params[:type_amount].nil?
      if params[:type_amount][0].include?('specific_lvl_1')
        ...
        if !params[:all_previous_accumulates].nil?
          ...
          @array_columns_prev_delivered_sum = params[:all_previous_accumulates].reject{|x| x=='1'}.map{|s| "SUM(IFNULL(" + s + "_"+ params[:type_amount][0] + ",0)) AS " + s + "_"+ params[:type_amount][0] }
        end
        ...
        if !params[:all_previous_accumulates].nil?
          ...
          @array_columns_prev_delivered_sum = params[:all_previous_accumulates].reject{|x| x=='1'}.map{|s| "SUM(IFNULL(" + params[:type_amount][0] + '_' + s + ",0)) AS " + params[:type_amount][0] + '_' + s}          
        end
      end
    end
    @treeOrders = ConsumptionCost.do_order(array_order, table_name, @array_columns_delivered, @array_columns_prev_delivered, @type_amount, @array_order_filters, @array_columns_delivered_sum, array_values_viewed)
