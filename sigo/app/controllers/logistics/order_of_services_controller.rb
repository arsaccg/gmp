  ...
  def display_orders
    ...
    os.each do |dos|
      ...
      when 'canceled'
        ...
        @action = "<a style='margin-right: 5px;' class='btn btn-view btn-xs' data-original-title='Ver Detalle' data-placement='top' onclick=javascript:load_url_ajax('/logistics/order_of_services/"+dos[0].to_s+"','content',{company_id:"+@com.to_s+"},null,'GET') rel='tooltip'><i class='fa fa-eye'></i></a>" + "<a style='margin-right: 5px;' class='btn btn-pdf btn-xs' data-original-title='Ver PDF' data-placement='top' href='/logistics/order_of_services/"+dos[0].to_s+"/order_of_service_pdf.pdf?company_id="+get_company_cost_center('company').to_s+"' rel='tooltip' target='_blank'><i class='fa fa-file'></i></a>"
