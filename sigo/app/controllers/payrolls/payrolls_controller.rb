class Payrolls::PayrollsController < ApplicationController
  ...
  def index
    @payslips = Payslip.select(:week).select(:code).select(:id).where(month: nil).where(cost_center_id: get_company_cost_center('cost_center')).group(:week)
    @month = Payslip.select(:month).select(:code).select(:id).where(week: nil).where(cost_center_id: get_company_cost_center('cost_center')).group(:month)
    render layout: false
  end
