class Valorization < ActiveRecord::Base
  ...
  def self.get_sub_itembybudgets(orderitem, valorization_id, budget_id)
    ...
    str_query = "SELECT  itembybudgets.id, 
        ...
        IFNULL(measured - (get_prev_valorizations(valorizations.valorization_date, itembybudgets.id) + valorizationitems.actual_measured), 0) AS 'saldo_metrado', 
        IFNULL((measured - (get_prev_valorizations(valorizations.valorization_date, itembybudgets.id) + valorizationitems.actual_measured)) * price, 0) AS 'saldo_costo',
        IFNULL(((get_prev_valorizations(valorizations.valorization_date, itembybudgets.id) + valorizationitems.actual_measured) / measured) * 100, 0) AS 'avance'
