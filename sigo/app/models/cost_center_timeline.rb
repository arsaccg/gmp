class CostCenterTimeline < ActiveRecord::Base
  belongs_to :cost_center

  # CostCenterTimeline.LoadTimeLine(1, Date.parse("31-01-2000"), Date.parse("13-02-2000"))
  def self.LoadTimeLine(p_cost_center, p_from, p_to)
    ...
    CostCenterTimeline.delete_all({cost_center_id: p_cost_center})
    # Load Link Period
    @dates.each do |x|
      @i +=1
      # Year changed
      ...
      CostCenterTimeline.create({:cost_center_id => p_cost_center, :date => x.date, :year => @year, :period => @year * 100 + @m, :week => @week, :day => @d })
      @d +=1
      @iDate +=1
      # Week changed
      if @i % 7 == 0
